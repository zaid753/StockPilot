...
const ChatRoom: React.FC<ChatRoomProps> = ({ chatParams, onBack }) => {
    ...
    const activeChatId = chatParams?.chatId || routeChatId || '';
    const activeChatTitle = (location.state as any)?.chatTitle || chatParams?.chatTitle || 'Chat';
    ...
    const handleBack = () => {
        if (onBack && onBack.toString().replace(/\s/g, '') !== '()=>{}') {
            onBack();
        } else {
            const role = userProfile?.role?.toLowerCase() || 'seller';
            navigate(`/${role}/network`);
        }
    };
    ...
    useEffect(() => {
        ...
        markMessagesAsSeen(activeChatId, user.uid);

        const unsubscribe = getMessagesStream(activeChatId, (fetchedMessages) => {
            ...
            if (unreadMessagesFromOthers.length > 0) {
                markMessagesAsDelivered(activeChatId, unreadMessagesFromOthers);
            }
        });
    ...
    const handleSendMessage = (e: React.FormEvent) => {
        ...
        if (newMessage.trim() && userProfile) {
            sendMessage(activeChatId, userProfile, newMessage.trim());
            setNewMessage('');
        }
    };
    ...
    return (
        <div className="h-screen flex flex-col p-4 md:p-8 container mx-auto">
            <header className="flex items-center justify-between mb-4 pb-4 border-b border-gray-200 dark:border-gray-700">
                <button onClick={handleBack} className="text-indigo-600 dark:text-indigo-400 hover:underline">
                    &larr; Back
                </button>
                <h1 className="text-xl font-bold text-gray-900 dark:text-white capitalize text-center">{activeChatTitle}</h1>
